...
export const OperationDetailsGql = ({
    ...
    const [schemaView, setSchemaView] = useState<TSchemaView>(
        (defaultSchemaView as TSchemaView) || TSchemaView.table
    );
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            .catch(
                (error) =>
                    new Error(
                        `Unable to load the graph details. Error: ${error.message}`
                    )
            )
            .finally(() => {
                setWorking(false);
    ...
    const loadGraph = async () => {
        ...
        try {
            const {
                graphqlTypes,
                schema,
            } = await graphqlService.getGraphqlTypesAndSchema(apiName);
    ...
    const getGraphType = (
        ...
        if (
            isScalarType(type) ||
            GraphqlDefaultScalarTypes.includes(typeName)
        ) {
        ...
        if (typeIndicators.length === 0) {
            ...
            return (
                <div>
                    {typeIndicators.map((indicator, index) =>
                        indicator === "[" ? (
    ...
    const getGraphReferences = (
        graph,
        graphqlTypes: TGraphqlTypes,
        references = []
    ) => {
        let type = graph.type;
        ...
        if (isScalarType(type) || GraphqlDefaultScalarTypes.includes(type.name))
            return references;
        ...
        if (!references.some((reference) => reference.name === type.name)) {
            ...
            if (
                ...
                Object.values(type[GraphqlFieldTypes.fields]).forEach((field) =>
                    getGraphReferences(field, graphqlTypes, references)
                );
            } else if (isUnionType(type)) {
                type[GraphqlFieldTypes.types].forEach(
                    (type: GraphQLObjectType) => {
                        !references.some(
                            (reference) => reference.name === type.name
                        ) && references.push(type);
                        Object.values(
                            type[GraphqlFieldTypes.fields]
                        ).forEach((field) =>
                            getGraphReferences(field, graphqlTypes, references)
                        );
                    }
        ...
        graph.args?.length > 0 &&
            graph.args.forEach((arg) =>
                getGraphReferences(arg, graphqlTypes, references)
            );
    ...
    const getGraphReferenceUrl = (typeName: string): string => {
        ...
        return routeHelper.getGraphDefinitionAnchor(
            apiName,
            graphType,
            graphName,
            typeName
    ...
    const getGraphReferenceId = (typeName: string): string => {
        ...
        return routeHelper.getGraphDefinitionReferenceId(
            apiName,
            graphType,
            graphName,
            typeName
                    ...
                    <div className={"operation-details-content"}>
                        ...
                        {graph.args?.length > 0 && (
                            <div className={"operation-response"}>
                                ...
                                >
                                    <TabList
                                        selectedValue={schemaView}
                                        onTabSelect={(
                                            e,
                                            data: { value: TSchemaView }
                                        ) => setSchemaView(data.value)}
                                    >
                                        <Tab value={TSchemaView.table}>
                                            Table
                                        </Tab>
                                        <Tab value={TSchemaView.schema}>
                                            Schema
                                        </Tab>
                                    </TabList>
                                </Stack>
