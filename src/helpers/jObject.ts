...
export type JElementType = "element" | "comment" | "cdata" | "text" | "document" | "template" | "question";
...
export class JObject {
    ...
    public static fromXml(xml: string, parseCallbacks?: {
        ...
        parser.on("text", (text: string, decodeEntities, contextGetter) => {
            ...
            /* Probably it makes sense to switch to a specialized element for text
             * (in next iteration?):
             *
             *   const element = new JText(checkForExpression(value));
             *   pushSibling(element);
             */
    ...
    private toFormattedXml(identation: number = 0, escapeCallbacks?: {
        ...
        switch (this.type) {
            ...
            case "element":
            case "template":
                const tag = Utils.join([this.ns, this.name], ":");
                ...
                this.attributes.forEach(attribute => {
                    ...
                    result += ` ${Utils.join([attribute.ns, attribute.name], ":")}="${value}"`;
                });
...
class XmlUtil {
    ...
    private static encodeRegex(): RegExp {
        return new RegExp(XmlUtil.chars.map((e) => {
            return e[0];
        }).join("|"), "g");
    }
    ...
    public static encode(str: string): string {
        return str.replace(XmlUtil.encodeRegex(), (s) => {
            return XmlUtil.encodeMap[s];
        });
    }

    public static decode(str: string): string {
        return str.replace(XmlUtil.decodeRegex(), function (s) {
            return XmlUtil.decodeMap[s];
        });
    }
}